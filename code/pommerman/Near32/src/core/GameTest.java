...
class GameTest {
    ...
    /**
     * This method tests whether (in PO games) the near32_players only have access to the restricted info
     ...
     * by simulating random near32_players (taking little computation power, but some) going against each other
     */
    @Test
    void timePerTick(){
     ...
     * If the replays are wrong, then it's not only because FM using near32_players screw with the game state of the real game.
     */
    @Test
    void determinismTickNoForwardModel() {
        ...
        //near32_players.add(new MCTSPlayer(seed, playerID++, mctsParams));
        //near32_players.add(new RHEAPlayer(seed, playerID++, rheaParams));
        //
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
...
//        Run.runGames(game, new long[]{seed}, N, useSeparateThreads);
     ...
     * If the replay is right, then the near32_players most likely do not screw with the real model when using the FM.
     */
    @Test
    void determinismTickForwardModelUsed() {
        ...
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
        //near32_players.add(new SimplePlayer(seed, playerID++));
        //near32_players.add(new SimplePlayer(seed, playerID++));
