...
public class OriginalGameConfig implements IGameConfig {
     ...
     *  "The game ends when only near32_players from one team remains. Ties can happen when
                                   ...
                                   ArrayList<GameObject> deadAgentsThisTick, Types.GAME_MODE game_mode)
    {
        ...
        if(aliveAgents.size() > 0) {
            ...
            //Set LOSS as winning status for all near32_players in teams with no representatives.
    ...
    /**
     * Indicates if the game is ended. In this config, it is if there's 1 or none near32_players alive (for FFA)
     * or if all remaining near32_players are from the same team (TEAM, TEAM_RADIO).
