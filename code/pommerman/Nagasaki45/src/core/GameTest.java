...
class GameTest {
    ...
    /**
     * This method tests whether (in PO games) the nagasaki45_players only have access to the restricted info
     ...
     * by simulating random nagasaki45_players (taking little computation power, but some) going against each other
     */
    @Test
    void timePerTick(){
    ...
    void copyDeterminism() {
        ...
        //Assign nagasaki45_players and run the game.
        game.setPlayers(players);
        ...
        //Run a single game with the nagasaki45_players
     ...
     * If the replays are wrong, then it's not only because FM using nagasaki45_players screw with the game state of the real game.
     */
    @Test
    void determinismTickNoForwardModel() {
        ...
        //nagasaki45_players.add(new MCTSPlayer(seed, playerID++, mctsParams));
        //nagasaki45_players.add(new RHEAPlayer(seed, playerID++, rheaParams));
        //
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
...
//        Run.runGames(game, new long[]{seed}, N, useSeparateThreads);
     ...
     * If the replay is right, then the nagasaki45_players most likely do not screw with the real model when using the FM.
     */
    @Test
    void determinismTickForwardModelUsed() {
        ...
        players.add(new SimplePlayer(seed, playerID++));
        players.add(new SimplePlayer(seed, playerID++));
        //nagasaki45_players.add(new SimplePlayer(seed, playerID++));
        //nagasaki45_players.add(new SimplePlayer(seed, playerID++));
