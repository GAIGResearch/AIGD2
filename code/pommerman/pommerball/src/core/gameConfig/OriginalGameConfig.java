...
public class OriginalGameConfig implements IGameConfig {
     ...
     *  "The game ends when only pommerball_players from one team remains. Ties can happen when
                                   ...
                                   ArrayList<GameObject> deadAgentsThisTick, Types.GAME_MODE game_mode)
    {
        ...
        if(aliveAgents.size() > 0) {
            ...
            //Set LOSS as winning status for all pommerball_players in teams with no representatives.
    ...
    /**
     * Indicates if the game is ended. In this config, it is if there's 1 or none pommerball_players alive (for FFA)
     * or if all remaining pommerball_players are from the same team (TEAM, TEAM_RADIO).
