...
public class Types {
    ...
    public static int MAX_GAME_TICKS = 1500;         //Maximum duration of the game.
    public static int BOMB_LIFE = 48;               //Ticks until a bomb explodes.
    public static int FLAME_LIFE = 10;               //Ticks until a flame dissappears.
    ...
    public static int BOARD_NUM_WOOD = 40;          //Number of wooden (destroyable) blocks for the level.
    public static int BOARD_NUM_ITEMS = 40;         //Number of items to put in level.
    public static int MAX_INACCESIBLE_TILES = 4;    //Number of inaccessible parts of the level allowed.
    public static int CORNER_DISTANCE = 7;          //Distance to the corner, in tiles, of the starting agent position.
    public static int BREATHING_SPACE = 4;          //Breathing space, L shaped tile section free at start around agent.

    // Visualization variables (used to display game for humans to see).
    public static int FRAME_DELAY = 30;
    ...
    public enum TILETYPE {
        ...
        {
            if      (key == PASSAGE.key) return ImageIO.GetInstance().getImage("img/grass.jpg");
            else if (key == RIGID.key) return ImageIO.GetInstance().getImage("img/plain_barrier.png");
            else if (key == WOOD.key) return ImageIO.GetInstance().getImage("img/chest.png");
            else if (key == BOMB.key) return ImageIO.GetInstance().getImage("img/football.png");
            else if (key == FLAMES.key) return ImageIO.GetInstance().getImage("img/red_card.png");
            else if (key == FOG.key) return ImageIO.GetInstance().getImage("img/fog.png");
            else if (key == EXTRABOMB.key) return ImageIO.GetInstance().getImage("img/goldilocks.png");
            else if (key == INCRRANGE.key) return ImageIO.GetInstance().getImage("img/trophy.png");
            ...
            else if (key == AGENT0.key) return ImageIO.GetInstance().getImage("img/red_player.png");
            else if (key == AGENT1.key) return ImageIO.GetInstance().getImage("img/blue_player.png");
            else if (key == AGENT2.key) return ImageIO.GetInstance().getImage("img/purple_player.png");
            else if (key == AGENT3.key) return ImageIO.GetInstance().getImage("img/yellow_player.png");
            else return null;
        }
