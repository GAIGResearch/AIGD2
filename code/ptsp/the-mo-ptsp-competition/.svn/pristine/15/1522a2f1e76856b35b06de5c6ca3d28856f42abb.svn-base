...
public class Map extends JComponent
{
    ...
    /**
     * Collectable fuel tank.
     */
    public static final char FUEL_TANK = 'F';
    ...
    /**
     * Symbol for a special damaging collision.
     */
    public static final char DAMAGE = 'D';

    /**
     * Symbol for a special elastic collision.
     */
    public static final char ELASTIC = 'L';

    /**
     * Symbol for a lava surface.
     */
    ...
    /**
     * List of the positions of the fuel tanks.
     */
    private LinkedList<Vector2d> m_fuelTankPos;
    ...
    private Map()
    {
        ...
        m_fuelTankPos = new LinkedList<Vector2d>();
    }
     ...
     */
    public Map(Game a_game, char[][] map, Vector2d startingPoint, LinkedList<Vector2d> wayPoints, LinkedList<Vector2d> fuelTanks)
    {
        this.m_game = a_game;
        ...
        {
            FuelTank newFT = new FuelTank(m_game, fuelTanks.get(i));
            m_mapChar[(int) newFT.s.x][(int)newFT.s.y] = Map.WAYPOINT;
            m_fuelTankPos.add(fuelTanks.get(i));
        }
    ...
    private void processData(int x, int y, char data)
    {
        ...
        else if(data == FUEL_TANK)
        {
            Vector2d newCol = new Vector2d(x,y);
            FuelTank ft = new FuelTank(m_game, newCol);
            if(m_game == null)
                m_game = new Game();
            m_game.addFuelTank(ft);
            m_fuelTankPos.add(new Vector2d(x,y));
        }
    }
    ...
    /**
     * Indicates if a given position in the grid is of a lava surface.
     * @param a_x x coordinate
     * @param a_y y coordinate
     * @return if the position is lava
     */
    public boolean isLava(int a_x, int a_y)
    {
        char inMap = m_mapChar[a_x][a_y];
        return isLava(inMap);
    }
    ...
    /**
     * Checks if the given character represents a lava surface
     * @param data character
     * @return true if it represents a lava surface.
     */
    private boolean isLava(char data)
    {
        if(data == Map.LAVA)
            return true;
        return false;
    ...
    /**
     * Gets the collision type in the map in a given position
     * @param a_x x coordinate of the position to check.
     * @param a_y y coordinate of the position to check.
     * @return Collision type, as defined in PTSPConstants.
     */
    public int getCollisionType(int a_x, int a_y)
    {
        if (a_x < 0) a_x = 0;
        if (a_y < 0) a_y = 0;
        if (a_x >= m_mapChar.length) a_x = m_mapChar.length-1;
        if (a_y >= m_mapChar[a_x].length) a_y = m_mapChar[a_x].length-1;

        char inMap = m_mapChar[a_x][a_y];
        return collisionType(inMap);
    }

    /**
     * Returns the collision type of a given character.
     * @param data character in the map.
     * @return Collision type, as defined in PTSPConstants.
     */
    private int collisionType(char data)
    {
        if(data == '@' || data == 'T' || data == '\u0000' || data == Map.EDGE)
            return PTSPConstants.NORMAL_COLLISION_TYPE;
        else if(data == Map.DAMAGE)
            return PTSPConstants.DAMAGE_COLLISION_TYPE;
        else if(data == Map.ELASTIC)
            return PTSPConstants.ELASTIC_COLLISION_TYPE;
    ...
    public LinkedList<Vector2d> getFuelTankPositions()
    {
        return m_fuelTankPos;
    }
