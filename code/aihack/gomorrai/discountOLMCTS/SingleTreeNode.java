package gomorrai.discountOLMCTS;

import core.game.StateObservationMulti;
import evodef.*;
import gomorrai.SimpleRMHC;
...
public class SingleTreeNode
{
    ...
    public static int sequenceLength = 20;
    public static GameActionSpaceAdapterMulti gameAdapter = null;
    public StateObservationMulti rootState;

    public static int ga_roller_evals = 20;
    public SimpleRMHC ga_roller = null;
    public int[] solution;
    ...
    public static void setupAdapter(StateObservationMulti state, int id, int oppID){
        gameAdapter = new GameActionSpaceAdapterMulti(state, sequenceLength, id, oppID);
    ...
    public void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(numIters < nIterations){
            ...
            if(selected.m_depth == MCTS_BUILD_DEPTH){
                //System.out.println("EA");
                delta = selected.execGAroller(state, elapsedTimer);
            }else {
    ...
    public SingleTreeNode treePolicy(StateObservationMulti state) {
        ...
        while (!state.isGameOver() && cur.m_depth < MCTS_BUILD_DEPTH)
        {
    ...
    private double execGAroller(StateObservationMulti state, ElapsedCpuTimer timer){
        double start_val = value(state);

        gameAdapter.setState(state);

        if(this.ga_roller == null){
            DefaultMutator mutator = new DefaultMutator(null);
            ga_roller = new SimpleRMHC().setMutator(mutator);
            ga_roller.setSamplingRate(1);
        }

        if (solution != null) {
            solution = SearchSpaceUtil.shiftLeftAndRandomAppend(solution, gameAdapter);
            ga_roller.setInitialSeed(solution);
        }

        solution = ga_roller.runTrial(gameAdapter, ga_roller_evals);

        return ga_roller.lastBestFitness - start_val;
    }
...
//        System.out.println(Arrays.toString(acts));
    ...
    public Types.ACTIONS opponentAction(int depth){
    ...
    public SingleTreeNode uct(StateObservationMulti state) {
        ...
        //get actions available to the opponent and assume they will do a random action
        acts[oppID] = opponentAction(this.m_depth+1);
    ...
    public double rollOut(StateObservationMulti state)
    {
        ...
        while (!finishRollout(state,thisDepth)) {
            ...
            for (int i = 0; i < no_players; i++) {
                if(i==oppID){
                    acts[i] = opponentAction(thisDepth);
                }else{
