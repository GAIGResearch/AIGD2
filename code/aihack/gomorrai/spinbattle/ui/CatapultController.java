...
import gomorrai.spinbattle.core.Planet;
import gomorrai.spinbattle.core.SpinGameState;
import gomorrai.spinbattle.core.Transporter;

import java.awt.event.MouseAdapter;
import java.awt.event.MouseEvent;

public class CatapultController extends MouseAdapter {
    Integer planetSelected;
    SpinGameState gameState;
    int playerId;


    public CatapultController setPlayerId(int playerId) {
        this.playerId = playerId;
        return this;
    }

    public CatapultController setGameState(SpinGameState gameState) {
    ...
    public CatapultController update() {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (planetSelected != null) {
            Planet source = gameState.planets.get(planetSelected);
            Transporter transit = source.getTransporter();
            // catapult from the current mouse location to the source position
            // but scale by a factor
            Vector2d velocity = source.position.copy().subtract(new Vector2d(e.getX(), e.getY()));
            velocity.mul(0.1);
            transit.catapultLaunch(source.position, velocity, playerId);
