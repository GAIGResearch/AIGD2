...
public class SpinGameState implements AbstractGameState {
    ...
    public SpinGameState setPlanets() {
        ...
        while (planets.size() < params.nPlanets && i++ < maxTries) {
            ...
            if (valid(planet)) {
                planet.setIndex(planets.size());
                planet.isSterile = (rnd.nextDouble() < params.sterileProbability);
                planet.isRebelious = (rnd.nextDouble() > params.rebelProbability);
                //System.out.println("Rebelius:" + planet.isRebelious);
                planets.add(planet);
