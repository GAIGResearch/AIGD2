...
import gomorrai.spinbattle.util.MovableObject;
...
public class Transporter {
    ...
    public Transporter copy() {
        ...
        transporter.initPayload = initPayload;
        if (mo != null)
    ...
    public Transporter next(VectorField vf) {
        if (mo != null) {
            ...
            if (prevPos != null && mo!=null && mo.s !=null) {
                distanceTravelled += prevPos.dist(mo.s);

                if (distanceTravelled > params.matingThreshold * diagonal()) {
                    payload += params.matingRatio * initPayload;
                }
            }
    ...
    private double diagonal(){
        return Math.sqrt(params.height^2+params.width^2);
    }

    private void payloadUpdate() {payload -= params.transportTax;}
